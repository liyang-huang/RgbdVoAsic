...
//       
//       Creation Date:  Wed Jun 28 22:10:38 2023
//       Version: 	r5p0
//
...
`ifdef POWER_PINS
module sram_lb_16b (VDDCE, VDDPE, VSSE, CENYA, WENYA, AYA, DYA, CENYB, WENYB, AYB,
...
`else
module sram_lb_16b (CENYA, WENYA, AYA, DYA, CENYB, WENYB, AYB, DYB, QA, QB, CLKA, CENA,
...
`endif
  ...
  reg NOT_TAA4, NOT_TAA3, NOT_TAA2, NOT_TAA1, NOT_TAA0, NOT_TDA15, NOT_TDA14, NOT_TDA13;
  ...
  reg NOT_TAB1, NOT_TAB0, NOT_TDB15, NOT_TDB14, NOT_TDB13, NOT_TDB12, NOT_TDB11, NOT_TDB10;
...
wire liyang_test = row_contention(AA_int, AB_int,  WENA_int, WENB_int);
  always @ CLKB_ begin
// If POWER_PINS is defined at Simulator Command Line, it selects the module definition with Power Ports
`ifdef POWER_PINS
    if (VDDCE === 1'bx || VDDCE === 1'bz)
...
`endif
 ...
 wire  CLKB_;
  ...
  buf B28(CENYB, CENYB_);
  buf B29(WENYB, WENYB_);
  buf B30(AYB[0], AYB_[0]);
  buf B31(AYB[1], AYB_[1]);
  buf B32(AYB[2], AYB_[2]);
  buf B33(AYB[3], AYB_[3]);
  buf B34(AYB[4], AYB_[4]);
  buf B35(AYB[5], AYB_[5]);
  buf B36(AYB[6], AYB_[6]);
  buf B37(AYB[7], AYB_[7]);
  buf B38(AYB[8], AYB_[8]);
  buf B39(AYB[9], AYB_[9]);
  buf B40(DYB[0], DYB_[0]);
  buf B41(DYB[1], DYB_[1]);
  buf B42(DYB[2], DYB_[2]);
  buf B43(DYB[3], DYB_[3]);
  buf B44(DYB[4], DYB_[4]);
  buf B45(DYB[5], DYB_[5]);
  buf B46(DYB[6], DYB_[6]);
  buf B47(DYB[7], DYB_[7]);
  buf B48(DYB[8], DYB_[8]);
  buf B49(DYB[9], DYB_[9]);
  buf B50(DYB[10], DYB_[10]);
  buf B51(DYB[11], DYB_[11]);
  buf B52(DYB[12], DYB_[12]);
  buf B53(DYB[13], DYB_[13]);
  buf B54(DYB[14], DYB_[14]);
  buf B55(DYB[15], DYB_[15]);
  buf B56(QA[0], QA_[0]);
  buf B57(QA[1], QA_[1]);
  buf B58(QA[2], QA_[2]);
  buf B59(QA[3], QA_[3]);
  ...
  buf B64(QA[8], QA_[8]);
  buf B65(QA[9], QA_[9]);
  buf B66(QA[10], QA_[10]);
  buf B67(QA[11], QA_[11]);
  buf B68(QA[12], QA_[12]);
  buf B69(QA[13], QA_[13]);
  buf B70(QA[14], QA_[14]);
  buf B71(QA[15], QA_[15]);
  buf B72(QB[0], QB_[0]);
  buf B73(QB[1], QB_[1]);
  buf B74(QB[2], QB_[2]);
  buf B75(QB[3], QB_[3]);
  ...
  buf B80(QB[8], QB_[8]);
  buf B81(QB[9], QB_[9]);
  buf B82(QB[10], QB_[10]);
  buf B83(QB[11], QB_[11]);
  buf B84(QB[12], QB_[12]);
  buf B85(QB[13], QB_[13]);
  buf B86(QB[14], QB_[14]);
  buf B87(QB[15], QB_[15]);
  buf B88(CLKA_, CLKA);
  buf B89(CENA_, CENA);
  buf B90(WENA_, WENA);
  buf B91(AA_[0], AA[0]);
  ...
  buf B115(DA_[14], DA[14]);
  buf B116(DA_[15], DA[15]);
  buf B117(CLKB_, CLKB);
  buf B118(CENB_, CENB);
  buf B119(WENB_, WENB);
  buf B120(AB_[0], AB[0]);
  ...
  buf B146(EMAA_[0], EMAA[0]);
  buf B147(EMAA_[1], EMAA[1]);
  buf B148(EMAA_[2], EMAA[2]);
  buf B149(EMAWA_[0], EMAWA[0]);
  buf B150(EMAWA_[1], EMAWA[1]);
  buf B151(EMASA_, EMASA);
  buf B152(EMAB_[0], EMAB[0]);
  buf B153(EMAB_[1], EMAB[1]);
  buf B154(EMAB_[2], EMAB[2]);
  buf B155(EMAWB_[0], EMAWB[0]);
  buf B156(EMAWB_[1], EMAWB[1]);
  buf B157(EMASB_, EMASB);
  buf B158(TENA_, TENA);
  buf B159(BENA_, BENA);
  buf B160(TCENA_, TCENA);
  buf B161(TWENA_, TWENA);
  buf B162(TAA_[0], TAA[0]);
  buf B163(TAA_[1], TAA[1]);
  buf B164(TAA_[2], TAA[2]);
  buf B165(TAA_[3], TAA[3]);
  buf B166(TAA_[4], TAA[4]);
  buf B167(TAA_[5], TAA[5]);
  buf B168(TAA_[6], TAA[6]);
  buf B169(TAA_[7], TAA[7]);
  buf B170(TAA_[8], TAA[8]);
  buf B171(TAA_[9], TAA[9]);
  buf B172(TDA_[0], TDA[0]);
  buf B173(TDA_[1], TDA[1]);
  buf B174(TDA_[2], TDA[2]);
  buf B175(TDA_[3], TDA[3]);
  buf B176(TDA_[4], TDA[4]);
  buf B177(TDA_[5], TDA[5]);
  buf B178(TDA_[6], TDA[6]);
  buf B179(TDA_[7], TDA[7]);
  buf B180(TDA_[8], TDA[8]);
  buf B181(TDA_[9], TDA[9]);
  buf B182(TDA_[10], TDA[10]);
  buf B183(TDA_[11], TDA[11]);
  buf B184(TDA_[12], TDA[12]);
  buf B185(TDA_[13], TDA[13]);
  buf B186(TDA_[14], TDA[14]);
  buf B187(TDA_[15], TDA[15]);
  buf B188(TQA_[0], TQA[0]);
  buf B189(TQA_[1], TQA[1]);
  buf B190(TQA_[2], TQA[2]);
  buf B191(TQA_[3], TQA[3]);
  buf B192(TQA_[4], TQA[4]);
  buf B193(TQA_[5], TQA[5]);
  buf B194(TQA_[6], TQA[6]);
  buf B195(TQA_[7], TQA[7]);
  buf B196(TQA_[8], TQA[8]);
  buf B197(TQA_[9], TQA[9]);
  buf B198(TQA_[10], TQA[10]);
  buf B199(TQA_[11], TQA[11]);
  buf B200(TQA_[12], TQA[12]);
  buf B201(TQA_[13], TQA[13]);
  buf B202(TQA_[14], TQA[14]);
  buf B203(TQA_[15], TQA[15]);
  buf B204(TENB_, TENB);
  buf B205(BENB_, BENB);
  buf B206(TCENB_, TCENB);
  buf B207(TWENB_, TWENB);
  buf B208(TAB_[0], TAB[0]);
  buf B209(TAB_[1], TAB[1]);
  buf B210(TAB_[2], TAB[2]);
  buf B211(TAB_[3], TAB[3]);
  buf B212(TAB_[4], TAB[4]);
  buf B213(TAB_[5], TAB[5]);
  buf B214(TAB_[6], TAB[6]);
  buf B215(TAB_[7], TAB[7]);
  buf B216(TAB_[8], TAB[8]);
  buf B217(TAB_[9], TAB[9]);
  buf B218(TDB_[0], TDB[0]);
  buf B219(TDB_[1], TDB[1]);
  buf B220(TDB_[2], TDB[2]);
  buf B221(TDB_[3], TDB[3]);
  buf B222(TDB_[4], TDB[4]);
  buf B223(TDB_[5], TDB[5]);
  buf B224(TDB_[6], TDB[6]);
  buf B225(TDB_[7], TDB[7]);
  buf B226(TDB_[8], TDB[8]);
  buf B227(TDB_[9], TDB[9]);
  buf B228(TDB_[10], TDB[10]);
  buf B229(TDB_[11], TDB[11]);
  buf B230(TDB_[12], TDB[12]);
  buf B231(TDB_[13], TDB[13]);
  buf B232(TDB_[14], TDB[14]);
  buf B233(TDB_[15], TDB[15]);
  buf B234(TQB_[0], TQB[0]);
  buf B235(TQB_[1], TQB[1]);
  buf B236(TQB_[2], TQB[2]);
  buf B237(TQB_[3], TQB[3]);
  buf B238(TQB_[4], TQB[4]);
  buf B239(TQB_[5], TQB[5]);
  buf B240(TQB_[6], TQB[6]);
  buf B241(TQB_[7], TQB[7]);
  buf B242(TQB_[8], TQB[8]);
  buf B243(TQB_[9], TQB[9]);
  buf B244(TQB_[10], TQB[10]);
  buf B245(TQB_[11], TQB[11]);
  buf B246(TQB_[12], TQB[12]);
  buf B247(TQB_[13], TQB[13]);
  buf B248(TQB_[14], TQB[14]);
  buf B249(TQB_[15], TQB[15]);
  buf B250(RET1N_, RET1N);
  buf B251(STOVA_, STOVA);
  buf B252(STOVB_, STOVB);
  buf B253(COLLDISN_, COLLDISN);
  ...
  assign DYB_ = RET1N_ ? (TENB_ ? DB_ : TDB_) : {16{1'bx}};
   `ifdef ARM_FAULT_MODELING
     sram_lb_16b_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .TQ(TQA_), .BEN(BENA_), .WEN(WENA_int), .Q_in(QA_int));
  `else
...
`endif
`timescale 1ns/1ps
module sram_lb_16b_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, BEN, TQ);
   output [15:0] Q_out;
   input [15:0] Q_in;
   input CLK;
   input [9:0] A;
