...
//
//       Instance Name:              sram_half_lb_16b
//       Words:                      320
//       Bits:                       16
//       Mux:                        16
//       Drive:                      6
...
//       
//       Creation Date:  Sat Jul 15 19:00:11 2023
//       Version: 	r5p0
//
...
`ifdef POWER_PINS
module sram_half_lb_16b (VDDCE, VDDPE, VSSE, CENYA, WENYA, AYA, DYA, CENYB, WENYB,
...
`else
module sram_half_lb_16b (CENYA, WENYA, AYA, DYA, CENYB, WENYB, AYB, DYB, QA, QB, CLKA,
 ...
 wire  CLKB_;
  ...
  assign DYB_ = RET1N_ ? (TENB_ ? DB_ : TDB_) : {16{1'bx}};
   `ifdef ARM_FAULT_MODELING
     sram_half_lb_16b_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .TQ(TQA_), .BEN(BENA_), .WEN(WENA_int), .Q_in(QA_int));
  `else
...
`endif
`timescale 1ns/1ps
module sram_half_lb_16b_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, BEN, TQ);
   output [15:0] Q_out;
   input [15:0] Q_in;
   input CLK;
   input [8:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(9'd182,4'd6,2'd1,2'd0);
   `endif
end
   task add_fault;
